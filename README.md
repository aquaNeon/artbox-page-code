...
| The indent | Webflow's `.w-checkbox` clearfix (`::before`/`::after`, `display: table`) is switched off. In a flex row each pseudo is an item, and the gap after the invisible `::before` put the box 8px in from the label above it |
...
A picture set inline into a heading grows out of nothing from its own middle,
and it **waits for its own line to land** rather than riding up with it. The line's mask carries the
...
The gesture is the home hero's, number for number: an iris from the middle over
`imgClipDuration` (1s, `--hero-in-open`) and a scale from `imgFrom` over
`imgDuration` (0.8s, `--hero-in-grow`), as two animations on the hero's
`--ease-inout-mask` curve. `TEXT.imgClip: ''` leaves the scale alone, and
...
A picture that travels leaves its own box uncovered at one edge for the length
of the move — across a row that reads as a band along the bottom. An overscale
fixes it and `data-fade-zoom="1.12"` is how you ask for one, but it is never
assumed: the cover crops the picture while it runs, and that is a decision for
the component, not for this module.
...
section visibly still full, and anything past about 0.7 reads as having
...
| `.meganav_feature_text`, `.button_main_wrap`, `.meganav_heading`, `.footer_link_wrap`, `[data-nav-content]` | the rows that stagger in, in DOM order. `.meganav_feature_text` is cut into lines with kugiri on every open and put back on close, and its lines rise out of their masks on the heading's curve at a menu's pace (`featureDuration` 0.8s, `featureStagger` 0.08). `.button_main_wrap` is opacity only over `buttonFade` (0.3s), cued off that sentence — `buttonOverlap` (0.45) of its rise — rather than off the swipe; with the sentence not on screen it falls back to the last row, after the swipe. These rows are the meganav's own reveal, not textAnim's: `data-text-anim-*` on anything in the sheet is read by nobody. `_fixture-meganav.html` is the live nav on the site's stylesheet, `?open=1` to open it on load |
